...
Debian mode is layered. Each tier is custom-made to work on vanilla Termux
and is protected by its own mechanism, so nothing above can reshape
anything below it.

| Tier | What | Source | Protection |
|---|---|---|---|
| 0. Platform (must-keep) | Termux's own packages; the hard core is Termux's `Essential: yes` set (36 on the test device: `apt`, `dpkg`, `bash`, `dash`, `coreutils`, `tar`, `termux-exec`, `termux-tools`, `glibc-runner`, `bash-glibc`, `coreutils-glibc`, ...) plus what it depends on (`openssl` via `coreutils`, `glibc` via `glibc-runner`) | Termux, updated in Termux mode | Debian mode never removes or changes a Termux package: the plan guard refuses any plan that does. apt already refuses to remove `Essential` packages |
| 1. Termux-backed identity | Debian names whose real content is Termux's: `libc6` (built), then `dash`, `debianutils`, `openssl` (the CLI), `ca-certificates` | generated by `dn-base-env.sh` from the installed Termux package | Debian's originals pinned to `-1` by origin. Content follows Termux, so security updates (`openssl`, `ca-certificates`) arrive with Termux |
| 2. Debian base, patched, frozen | `base-files`, `base-passwd`, `debconf`, `cdebconf`, `mawk` -- what makes every other package see "Debian" (`main`'s `bootstrap-base.sh` set) | Debian, through the pipeline, once | `apt-mark hold` after install; refreshed only deliberately (unhold, upgrade through the pipeline, re-verify, re-hold) |
| 3. Packages | everything else | Debian, through the pipeline | normal apt |

Tier 1 naming rule: a Termux-backed identity must not share a name with an
installed Termux package, or installing it crossgrades Termux's copy away.
`libc6` can keep Debian's name because Termux has no `libc6`; the others
are built as e.g. `dn-dash` with `Provides: dash (= <termux version>)`.
Note `openssl` means different things: Termux's `openssl` is the library
(tier 0), Debian's is the CLI tool; Debian's library is `libssl3t64` and
comes from Debian in tier 3.
...
that has left the index stops holding silently. A hold lives in dpkg's own
selection state. The cost is strictness: a package needing a newer base
(`debconf (>= X)`) is refused with a "held packages" error until the base
is refreshed.
